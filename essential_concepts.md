...
* Application Programming Interface (_API_)
  * defines interfaces by which one piece of software communicates with another at the _source level_
* Application Binary Interface (_ABI_)
  * defines binary interface between pieces of software on a particular architecture (e.g. x86-64)
  * concerned with e.g.
    * calling conventions, byte ordering, register use, system call invocation, _(cont'd)_
...
* **regular files**
  ...
  * **inodes**
    ...
    * _fully qualified_ / _absolute pathname_ - starting at `root` "/" directory
  ...
  * **hard links** and **symbolic links**
    ...
    * _symlinks_ - has its own inode and data chunk, containing the complete pathname of linked-to file
...
* _sector_ - smallest addressable unit on block device; pysical attribute of device
...
* _per-process namespaces_ by default
...
* **ELF** _Excecutable and Linkable Format_
...
* uid 0 = _root_
* each user has a _primary_ or _login group_ (in `/etc/passwd`); _supplemental groups_ are in `/etc/groups`
